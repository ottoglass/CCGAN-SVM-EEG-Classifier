...
class CCGAN():
    ...
    def train(self, X_train,y_train,epochs, batch_size=128, sample_interval=50):
        X_train=np.pad(X_train,[[0,0],[7,0],[3,0],[0,0]],'constant')/255
        # Adversarial ground truths
        valid = np.ones((batch_size, 6, 16, 1))
        fake = np.zeros((batch_size, 6, 16, 1))
    ...
    def save_model(self):
        ...
        save(self.generator, "ccgan_bci_generator")
        save(self.discriminator, "ccgan_bci_discriminator")
...
if __name__ == '__main__':
    ...
    x_train=np.array(imread_collection(x_sample))
    y_train=np.array(y_sample)
    ccgan = CCGAN()
    ccgan.train(x_train,y_train,epochs=80000, batch_size=100, sample_interval=200)